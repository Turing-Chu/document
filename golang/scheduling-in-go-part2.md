...
随着时间的推移，在操作系统级别将IO/阻塞任务转为CPU密集型任务的能力使我们在利用更多PU容量方面获得了巨大胜利。所以，在具有虚拟内核的操作系统上无需更多线程。在每个虚拟内核只有一个OS线程的情况下期待完成更多工作（CPU和IO/阻塞绑定）是合理的。网络应用以及其他不需要阻塞OS线程的系统调用都可以这样做。
